    ...
    stages {
        ...
        stage('Run Test') {
            steps {
                ...
                powershell "docker-compose run robot robot --include Param --variable auction_type:${env.Type} --variable realty:${env.Realty} --variable location:${env.Location} --variable size:${env.Size} --variable starttime:${env.BUILD_TIMESTAMP_PATH} --variable search_limit:${env.Search_limit} --variable search_type:${env.Search_type} execution/sreality.robot"                
            }
        }
        stage('Send E-mail') {
            when{
